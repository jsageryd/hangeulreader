...
public class CharacterRenderer {
	...
	public static BufferedImage makeCharacterImage(char c, int width, int height, Font font){
		...
		g2d.setColor(new Color(255,255,255));
		g2d.fillRect(0, 0, width, height);
		drawCharacter(g2d, c, width / 2, height / 2, (int) ((width < height ? width : height)*0.8), font);
		return img;
	}
	...
	public static int getFontSize(Graphics2D g2d, Font font, int maxSide){
		char c = 'M';
		Font f = font.deriveFont(10f);
		FontMetrics fm = g2d.getFontMetrics(f);
		int width = fm.charWidth(c);
		int height = fm.getHeight();
		int side = width < height ? width : height;
		int fontsize = (maxSide * 10) / side;
		return fontsize < 1 ? 1 : fontsize;
	}

	/**
	 * Calculates the maximum font size that can be used for the specified string to fit within the specified width.
	 * @param string	the string to fit
	 * @param g2d	the <code>Graphics2D</code> to use for measurements (nothing will be drawn on it)
	 * @param font	the font to use
	 * @param maxWidth	the width limit
	 * @return
	 */
	public static int getBoxFitFontSize(String string, Graphics2D g2d, Font font, int maxWidth, int maxHeight){
		Font f = font.deriveFont(10f);
		FontMetrics fm = g2d.getFontMetrics(f);
		int width = fm.stringWidth(string);
		int height = fm.getHeight();
		int fontsizew = (maxWidth * 10) / (width < 1 ? 1 : width);
		int fontsizeh = (maxHeight * 10) / (height < 1 ? 1 : height);
		int fontsize = fontsizew < fontsizeh ? fontsizew : fontsizeh;
		return fontsize < 1 ? 1 : fontsize;
	}
